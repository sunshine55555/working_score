...
class WorkspaceCreditScore():
    def __init__(self):
        ...
        self.__stability = {
            'worktime': {(0, 1): [1,2,0.8], (1, 2): [4, 5,2],(2, 3): [7,9,3], (3, 4): [9,14,5], (4, 6): [13,15,8], (6, 8): [15,16,13], (8, 10): [16,14,15], (10, 100): [17,8,18]},
            'salary': {0: [1,1,1], 1: [2,2.2,2.4], 2: [3,3.6,3.8], 3: [4,5.2,5.6], 4: [5,7.4,8], 5: [7,10.4,11.2], 6: [8,12,12.8], 7: [9,13.4,14.4], 8: [10,13,16], 9: [11,14.2,15.4],10: [12,15.6,16.8], 11: [14,18.2,19.6]},
            "insurance_edu_time": {(0, 0.4): 4.5, (0.4, 0.8): 5.4, (0.8, 1): 8.1},
            "time_num": {(0, 6): [2.24, 1.28], (6, 12): [2.88, 1.6], (12, 18): [3.2, 2.24] , (18, 24): [3.68, 4.8] , (24, 2000): [4, 6.08]},
        ...
        self.__honesty = {
            ...
            "bank_black": {0:0,1:5.6,2:9.8,3:14},
    ...
    def __stability_score(self):
        ...
        for i in self.__stability['age']:
            if not self.age:
                ...
                break
        stability_score = (worktime_score + salary_score + insurance_edu_time_score + time_num_score + marry_score + position_score + age_score) * self.coe
    ...
    def __honesty_score(self):#
        ...
        # 金融黑名单
        for i in self.__honesty['bank_black']:
            if self.bank_black == "" :
                bank_black_score += self.__honesty['bank_black'][3]

                print('金融：',bank_black_score)
                break
            elif not self.bank_black:
                self.coe *= 0.85
                bank_black_score += self.__honesty['bank_black'][0]
                print('hello')
                break
        if self.bank_black:

            if self.bank_black == 1:
                self.coe *= 0.95
                bank_black_score += self.__honesty['bank_black'][1]

            elif self.bank_black == 2:
                self.coe *= 1
                bank_black_score += self.__honesty['bank_black'][2]

            elif self.bank_black == 3:
                bank_black_score += self.__honesty['bank_black'][3]
            else:
                print('金融黑名单数字传输错误，请重新输入')
                bank_black_score += self.__honesty['bank_black'][4]
